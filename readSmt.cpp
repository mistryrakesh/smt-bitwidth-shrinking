...
void Node::replaceSucc(Node *oldNode, Node *newNode) {
    for (vector<Node *>::iterator it = succs.begin(); it != succs.end(); ++it) {
        if (*it == oldNode) {
            if (logFlag)
                logFile << "Replacing successor node: " << oldNode->getName() << " with new node: " << newNode->getName() << endl;
            *it = newNode;
        }
    }
}
...
void replaceUnhandledNodes(map<string, Node *>& nMap, map<string, string>& replaceNodesMap) {
    for (map<string, Node *>::iterator it = nMap.begin(); it != nMap.end(); ++it) {
        Node *node = it->second;
        if (operatorsWhichGetReplaced.find(node->getOper()) != operatorsWhichGetReplaced.end()) {
            node = replaceNodeWithVariable(node, replaceNodesMap);
        }
...
int main(int argc, char **argv) {
    ...
    if (isShrinkable) {
        ...
        replaceUnhandledNodes(nodeMap, replaceNodesMap);
